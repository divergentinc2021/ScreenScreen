...
export default function SettingsPanel() {
  ...
  const [calendarAccounts, setCalendarAccounts] = useState<{ name: string; partition: string }[]>(settings.calendar?.accounts || [])
  ...
  useEffect(() => {
    ...
    setCalendarAccounts(settings.calendar?.accounts || [])
  }, [settings])
  ...
  const handleSignInGoogle = async () => {
    setSigningIn(true)
    try {
      const partition = `persist:cal-${Date.now()}`
      const result = await window.api.calendarSignIn(partition)
      if (result.success && result.email) {
        setCalendarAccounts([...calendarAccounts, { name: result.email, partition }])
  ...
  const handleRemoveCalendar = async (index: number) => {
    const account = calendarAccounts[index]
    try {
      await window.api.calendarRemoveSession(account.partition)
    } catch { /* ignore */ }
    setCalendarAccounts(calendarAccounts.filter((_, i) => i !== index))
  }
    ...
    <div className="flex-1 flex flex-col p-8 max-w-xl overflow-auto">
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="bg-surface rounded-xl p-5 border border-border">
          ...
          <p className="text-xs text-muted mb-4">
            Sign in with any Google account to sync your calendar. Add multiple accounts to see all your meetings in one place.
          </p>

          <div className="flex flex-col gap-3">
            {/* Connected accounts */}
            {calendarAccounts.map((account, i) => (
            ...
            {/* Settings (show when accounts are added) */}
